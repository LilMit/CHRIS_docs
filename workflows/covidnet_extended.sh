...
# Set variables so you don't have to copy/paste. We will uncomment the printf/reads below for the demo.
MY_URL="https://raw.githubusercontent.com/reedh13/filefetch-example/master/my_links.txt"
MY_TYPES="dcm"

# printf "Enter URL to text file:\n
# read MY_URL

# printf "Enter list of desired file types: (example: pdf,jpeg,png)\n"
# read MY_TYPES
...
declare -a a_WORKFLOWSPEC=(
    ...
    "2*_n:5*_n:l1|
    reedh13/pl-image_inversion: ARGS;
                                --inputFile=sample.png;
                                --previous_id=@prev_id"

    "1*_n:6*_n:l1|
    mchandarana/pl-pfdcm_tagextract:3.0.0 ARGS;
                                --inputFile=@image[_n];
                                --outputFileType=raw,json,dict,col,csv;
                                --title=tags_pfdcm;
                                --previous_id=@prev_id"

    "1*_n:7*_n:l1|
    mchandarana/pl-newdcmdump:2.0.0 ARGS;
                                --inputFile=@image[_n];
                                --title=tags_dcmtk;
                                --previous_id=@prev_id"
)
...
title -d 1 "Start constructing the Feed by POSTing the root FS node..."
    ...
    #\\\\\\\\\\\\\\\\\\
    # Core logic here ||
    plugin_run          "0:0"   "a_WORKFLOWSPEC[@]"   "$CUBE"  ROOTID $sleepAfterPluginRun \
                        "@my_txturl=$MY_URL;@my_types=$MY_TYPES;@my_token=$MY_TOKEN" && id_check $ROOTID
...
title -d 1 "Building and Scheduling workflow..."
    ...
    for image in "${a_lungCT[@]}" ; do
        ...
        plugin_run  ":4" "a_WORKFLOWSPEC[@]" "$CUBE" ID4 $sleepAfterPluginRun \
                    "@prev_id=$ID3;@patientID=$ID3-12345" && id_check $ID4
        digraph_add "GRAPHVIZBODY" "GRAPHVIZBODYARGS" ":3;$ID3" ":4;$ID4" \
                    "a_WORKFLOWSPEC[@]"

        # image_inversion
        plugin_run  ":5" "a_WORKFLOWSPEC[@]" "$CUBE" ID5 $sleepAfterPluginRun \
                    "@prev_id=$ID2" && id_check $ID5
        digraph_add "GRAPHVIZBODY" "GRAPHVIZBODYARGS" ":2;$ID2" ":5;$ID5" \
                    "a_WORKFLOWSPEC[@]"

        # TODO: SWAP ORDER OF 5 and 6?

        # pfdicom_tag_extract
        plugin_run  ":6" "a_WORKFLOWSPEC[@]" "$CUBE" ID6 $sleepAfterPluginRun \
                    "@prev_id=$ID1;@image[_n]=$image" && id_check $ID6
        digraph_add "GRAPHVIZBODY" "GRAPHVIZBODYARGS" ":1;$ID1" ":6;$ID6" \
                    "a_WORKFLOWSPEC[@]"

        # newdcmdump
        plugin_run  ":7" "a_WORKFLOWSPEC[@]" "$CUBE" ID7 $sleepAfterPluginRun \
                    "@prev_id=$ID1;@image[_n]=$image" && id_check $ID7
        digraph_add "GRAPHVIZBODY" "GRAPHVIZBODYARGS" ":1;$ID1" ":7;$ID7" \
                    "a_WORKFLOWSPEC[@]"
        ...
        if (( b_printReport || b_printJSONprediction )) ; then
            # get list of file resources for the prediction plugin (ID3)
